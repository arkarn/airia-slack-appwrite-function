...
def handle_slack_event(body, headers, context=None):
    ...
    # Debug logging
    if context:
        context.log(f"Event type: {event_type}, bot_id: {event.get('bot_id')}, subtype: {event.get('subtype')}")
    ...
    if event_type == "app_mention":
        ...
        if slack_token:
            ...
            if context:
                context.log(f"Scheduled message for thread_ts: {thread_ts}")
    ...
    if event_type == "message" and not event.get("subtype"):
        ...
        if context:
            context.log(f"Message event - thread_ts: {event.get('thread_ts')}, ts: {event.get('ts')}, lookup: {lookup_thread_ts}")
        
        if not event.get("bot_id"):
            # Human replied - cancel scheduled message
            if context:
                context.log(f"Attempting to cancel for thread_ts: {lookup_thread_ts}")
            cancel_scheduled_message(lookup_thread_ts, channel, context)
...
def cancel_scheduled_message(thread_ts, channel, context=None):
    slack_token = os.environ.get("SLACK_BOT_TOKEN")
    doc = get_pending_schedule(thread_ts)
    
    if context:
        context.log(f"Found doc for thread_ts {thread_ts}: {doc is not None}")
    
    if doc and slack_token:
        # Cancel the scheduled message
        resp = requests.post("https://slack.com/api/chat.deleteScheduledMessage",
            headers={"Authorization": f"Bearer {slack_token}"},
            json={"channel": channel, "scheduled_message_id": doc["scheduled_message_id"]})
        
        if context:
            context.log(f"Cancel response: {resp.json()}")
        ...
        context.log(f"Cannot cancel - doc: {doc is not None}, token: {slack_token is not None}")
